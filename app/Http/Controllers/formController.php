...
class formController extends Controller
{
    public function form(Request $request)
    {
        ...
        if ($validation->fails()) {
            ...
            if (!file_exists("$tmpfileUrl/zip")) {
                mkdir("$tmpfileUrl/zip");
            }

            File::copyDirectory(storage_path('test'), $url);

            //Funkcja kopiująca pliki oraz wypełniająca je danymi z formularza
            $this->update($dataForm, $url, ['SunGroup.html', 'SunGroup.txt', 'SunGroup.rtf']);

            //Tworzenie tymczasowego pliku zip i włożenie do niego plików
